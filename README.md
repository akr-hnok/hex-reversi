...
- **シミュレーション検証に基づく手番ローテーション（順列交代方式）**
  - 多人数リバーシで問題となる「最終手番が毎ラウンドの石を総取りして100%勝利する」致命的構造バイアスを解消。
  - 全6順列を巡回させる「順列交代方式（Permutation Cycle）」により、待たされ感の原因だった「4手待ち」を完全排除（最大待ち手数3手）。1番手・2番手・3番手の担当機会を数学的に完全均等化。
  - さらに「開始席・回転方向の対局ごとランダム化」により、席順による不公平を徹底排除。
